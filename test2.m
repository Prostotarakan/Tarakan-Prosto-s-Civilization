...
while age<300
   ...
   if war_call>0
       ...
       ages=Historic(n-war_call+1:n,4)-Historic(n-war_call+1:n,1);
   else cou=0;
   end
   for i=1:h
   [x,y,~] = nearit(i,Map1,i);
   if x
       ...
       if c %при войне!  
           ...
           %все проседает...
           Nat(i).econ=Nat(i).econ*(10-length(enemy))/10;
           Nat(i).war=Nat(i).war*(12-length(enemy))/10;
           Nat(i).sci=Nat(i).sci*(10-length(enemy))/10;
           Nat(i).flot=Nat(i).flot*(10-length(enemy))/10;
           ...
           if length(enemy)==1 && enemy==i && ageen>5
               Nat(i).war=0.1;
           end
           ...
           if Nat(i).war<0.15 || x==0 || Nat(i).econ<0.15 || sum(ageen)>50
               %проигрыш
               ...
               if coco
                  buf=Historic(coco,:);
                  Historic(coco,:)=[];
                  Historic=[Historic;buf];
                  war_call=length(coco);
               end
